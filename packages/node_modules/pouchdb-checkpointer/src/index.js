...
class CheckpointerInternal {
  ...
  async writeCheckpoint(checkpoint, session) {
    ...
    await this.#updateTarget(checkpoint, session);
    return this.#updateSource(checkpoint, session);
  }

  async #updateTarget(checkpoint, session) {
    if (this.opts.writeTargetCheckpoint) {
      return await this.#updateCheckpoint(this.target, this.id, checkpoint,
        session, this.returnValue);
    } return true;
  }

  async #updateSource(checkpoint, session) {
    if (this.opts.writeSourceCheckpoint) {
      try {
        return await this.#updateCheckpoint(this.src, this.id, checkpoint,
          session, this.returnValue);
      } catch (err) {
  ...
  async #updateCheckpoint(db, id, checkpoint, session, returnValue) {
    //retrieve checkpoint doc from db or create a new one
    const doc = await this.#initCheckpointDoc(db, id, session);
    ...
    if (returnValue.cancelled) {
      ...
      try {
        ...
        if (err.status === 409) {
          ...
          return this.#updateCheckpoint(db, id, checkpoint, session, returnValue);
        }
          throw err;
      }
  }

  async #initCheckpointDoc(db, id, session) {
    try {
